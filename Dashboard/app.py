...
st.set_page_config(page_title='Examining Depression Using Health Care Data',
                   page_icon=':rain_cloud:', layout='centered')
...
with st.expander('Look at the percentage of depression among categorical features'):
    ...
    cat_option = st.selectbox(
        'Select a feature to examine', cat_cols, key='cat_cols1')
...
with st.expander('Look at depression in numerical features plotted together'):
    ...
    num_optionX = st.selectbox(
        'Select a feature for the X-axis', num_cols, key='x1')
    num_optionY = st.selectbox(
        'Select a feature for the Y-axis', num_cols, key='y1')
...
with st.expander('Look at depression in a numerical feature alone or add a categorical feature'):
    ...
    num_optionX2 = st.selectbox(
        'Select a numerical feature for the X-axis', num_cols, key='x2')
    cat_optionY = st.selectbox(
        'Select a categorical feature for the Y-axis if desired', none_included, key='y2')
...
with st.expander('Look at depression with prescription data'):
    ...
    def percentage_plot_rx(col):
        ...
        # Creates a temporary dataframe to get the percentages
        temp_df = df.groupby(f'Rx {col}')[
            'Depression'].value_counts(normalize=True)
        temp_df = temp_df.mul(100).rename('Percent').reset_index()
        temp_df['Percent'] = temp_df['Percent'].round(decimals=1)
        ...
        st.markdown('**Explaination of the feature selected:**')
        st.markdown(
            f'**- {col}: Percent depression by {col} prescription and days the person has been prescribed {col} medication**')
        return fig
